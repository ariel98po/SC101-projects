...
we built today, including Tree, LinkedList, and
    ...
    def enqueue(self, element):
        if self.pq.value is None:
            ...
            if self.pq.value[1] > element[1]:
                ...
                new_node = LinkedList(element, self.pq)
                self.pq = new_node
            else:
                # New node in between
                current = self.pq
                while current.next is not None:
                    if current.value[1] <= element[1] < current.next.value[1]:
                        new_node = LinkedList(element, current.next)
                        current.next = new_node
                        break
                    current = current.next
                # New node at the end
                if current.next is None:
                    current.next = LinkedList(element, None)
